...
const UpdateModel = () => {
  ...
  const submit = async () => {
    ...
    if (validateForm()) {
      try {
        ...
        await amettAPI.patch(`/equipment-models/${modelId}`, formData, {
  ...
  useEffect(() => {
    const fetchModel = async () => {
      try {
        const response = await amettAPI.get(`/equipment-models/${modelId}`);
        setModel(response.data.data.equipmentModel);
        setModelName(response.data.data.equipmentModel.name);
        setType(response.data.data.equipmentModel.type._id);
        setManufacturer(response.data.data.equipmentModel.manufacturer._id);
  ...
  return (
    <div className="grid grid-cols-5 gap-4">
      ...
      {isSuccessAlertShown ? (
        <SuccessNotification
          msg={"Success! The model has been saved"}
