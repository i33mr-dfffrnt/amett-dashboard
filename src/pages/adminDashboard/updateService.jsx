...
const UpdateService = () => {
  ...
  const submit = async () => {
    ...
    if (validateForm()) {
      try {
        ...
        await amettAPI.patch(`/services/${serviceId}`, formData, {
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        const response = await amettAPI.get(`/services/${serviceId}`);
        ...
        setType(response.data.data.service.type._id);
  ...
  return (
    <div className="grid grid-cols-5 gap-4">
      ...
      {isSuccessAlertShown ? (
        <SuccessNotification
          msg={"Success! The service has been saved"}
