  ...
  useEffect(() => {
    const fetchTypes = async () => {
      try {
        ...
        setTypesList(response.data.data.serviceTypes);
        setFilteredList(response.data.data.serviceTypes);
      } catch (error) {}
    };
    fetchTypes();
    setIsCheck([]);
        ...
        <td className="flex items-center justify-center px-6 py-4 space-x-3 ">
          <button
            onClick={() => {
              ...
              setSubmitFunction(() => () => {
                return amettAPI.delete(`/service-types/${el._id}`);
              });
              setModalMsg(
                `Are you sure you want to delete ${el.name}? All associated services will be deleted as well`
              );
        ...
        <div className="flex flex-row justify-end gap-3 my-4">
          ...
          <button
            onClick={() => {
              ...
              setModalMsg(
                `Are you sure you want to delete ${isCheck.length} items? All associated services will be deleted as well`
              );
              setModalSuccessMsg(`${isCheck.length} items were deleted successfully!`);
              setModalErrMsg(`Something went wrong! Please try again later`);
              setSubmitFunction(() => () => {
                return amettAPI.delete(`/service-types`, {
