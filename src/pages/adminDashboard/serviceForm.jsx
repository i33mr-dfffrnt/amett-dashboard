...
const ServiceForm = () => {
  ...
  const validateForm = () => {
    if (ServiceName.trim() === "") {
      setErrorMessage("Please input Service name");
      return false;
    } else if (type.trim() === "") {
      setErrorMessage("Please choose a Service type ");
      return false;
      // } else if (manufacturer.trim() === "") {
      //   setErrorMessage("Please choose a Service manufacturer ");

      //   return false;
    } else if (description.trim() === "") {
      setErrorMessage("Please input Service description");
      return false;
      // } else if (manufacturer.trim() === "") {
  ...
  const submit = async () => {
    ...
    if (validateForm()) {
      try {
        ...
        await amettAPI.post(`/services`, formData, {
  ...
  useEffect(() => {
    ...
    const fetchTypes = async () => {
      try {
        const response = await amettAPI.get(`/service-types`);
        setTypeList(response.data.data.serviceTypes);

        setType(response.data.data.serviceTypes[0]._id);
      } catch (error) {}
    };
    // fetchManufacturers();
    fetchTypes();
  ...
  return (
    <div className="grid grid-cols-5 gap-4">
      ...
      {isSuccessAlertShown ? (
        <SuccessNotification
          msg={"Success! Your new Service has been created"}
          ...
          <div className="col-span-5 ">
            {imageSrc && (
              <div className="mx-auto max-w-2xl px-4 pt-10 pb-16 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8 lg:pt-8 lg:pb-24">
                ...
                <div className="w-full rounded-lg  Service-carousel">
                  How it will look like:
                  <div className="Service-carousel-slide rounded-lg">
                    <img
                      src={previewUri.url}
                      alt=".."
