...
const ManageEquipmentManufacturers = () => {
  ...
  const sortList = ["Newest", "Oldest", "Manufacturer A-Z"];
  ...
  const sortManufacturers = (method) => {
    if (method === "Newest") {
      setFilteredList(
  ...
  const filterBySearch = (event) => {
    ...
    var updatedList = [...manufacturersList];
  ...
  const manufacturers = filteredList.map((el) => {
    return (
        ...
        <td className="flex items-center justify-center px-6 py-4 space-x-3 ">
          <button
            onClick={() => {
              ...
              setSubmitFunction(() => () => {
                return amettAPI.delete(`/equipment-manufacturers/${el._id}`);
              });
              setModalMsg(
          ...
          <Link
            to={`/admin-dashboard/update-manufacturer/${el._id}`}
            className=" flex w-30 text-sm items-center justify-center rounded-3xl border border-transparent  drop-shadow-lg bg-white py-1 px-1  font-semibold text-white "
          >
      ...
      <div className="col-span-4 mt-10 ">
        ...
        <div className="flex flex-row justify-end gap-3 my-4">
          <h3 className="flex w-30 text-sm items-center justify-center rounded-sm border border-transparent  py-1 px-4  font-semibold ">
            {`${filteredList.length} manufacturers were found`}
          </h3>
          ...
          <button
            onClick={() => {
              ...
              setModalErrMsg(`Something went wrong! Please try again later`);
              setSubmitFunction(() => () => {
                return amettAPI.delete(`/equipment-manufacturers`, {
          ...
          <SortDropdown sortList={sortList} sort={sortManufacturers} />
