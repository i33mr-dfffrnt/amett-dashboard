...
const ManageRequests = () => {
  ...
  const sortList = ["Newest", "Oldest", "Requests A-Z", "Type A-Z"];
  const [filteredList, setFilteredList] = new useState([]);

  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await amettAPI.get(`/requests`);
        console.log(response.data.data);
        setServicesList(response.data.data.requests);
        setFilteredList(response.data.data.requests);
      } catch (error) {}
    };
    fetchServices();
    setIsCheck([]);
  ...
  const services = filteredList.map((el) => {
    // Convert the date to the browser's local date format
    ...
    return (
      <tr className="bg-white border-b hover:bg-gray-50" key={el._id}>
        ...
        <td className="px-6 py-4 max-w-sm ellipsis">
          <a href={`mailto:${el.email}`} className="text-blue-500 hover:underline">
            {el.email}
          </a>
        </td>
        <td className="px-6 py-4 max-w-sm ellipsis">
          <a href={`tel:${el.phone}`} className="text-blue-500 hover:underline">
            {el.phone}
          </a>
        </td>
        <td className="px-6 py-4 max-w-sm ellipsis">{formattedDate}</td>
        <td className="flex items-center justify-center px-6 py-4 space-x-3">
          <button
            onClick={() => {
              ...
              setSubmitFunction(() => () => {
                return amettAPI.delete(`/requests/${el._id}`);
              });
  ...
  return (
    <div className=" grid grid-cols-5 gap-4">
      ...
      <div className="col-span-4 mt-10 ">
        <h2 className="text-xl sm:text-4xl mb-5 mt-2 playfairDisplay-font font-bold">Requests</h2>
        <div className="flex flex-row justify-end gap-3 my-4">
          <h3 className="flex w-30 text-sm items-center justify-center rounded-sm border border-transparent  py-1 px-4  font-semibold ">
            {`${filteredList.length} requests were found`}
          </h3>
          ...
          <button
            onClick={() => {
              ...
              setModalErrMsg(`Something went wrong! Please try again later`);
              setSubmitFunction(() => () => {
                return amettAPI.delete(`/requests`, {
