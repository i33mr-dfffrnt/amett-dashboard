...
const UpdateManufacturer = () => {
  ...
  useEffect(() => {
    const fetchManufacturer = async () => {
      try {
        const response = await amettAPI.get(`/equipment-manufacturers/${manufacturerId}`);
  ...
  const validateForm = () => {
    if (manufacturerName.trim() === "") {
      setErrorMessage("Please input manufacturer name");
      return false;
    } else return true;
  };
  ...
  const submit = async () => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        await amettAPI.patch(`/equipment-manufacturers/${manufacturerId}`, formData, {
  ...
  return (
    <div className="grid grid-cols-5 gap-4">
      ...
      {isSuccessAlertShown ? (
        <SuccessNotification
          msg={"Success! Your new manufacturer has been created"}
      ...
      <div className="col-span-4 mt-10 ">
        ...
        <form className="grid grid-cols-5  gap-4 text-md lg:text-xl playfairDisplay-font  mr-3  bg-baseGray px-10 lg:px-20 xl:px-32 py-10">
          <h5 className="">Manufacturer Name*</h5>
          <input
            type="text"
            name="name"
          ...
          <h5 className="">Manufacturer Photo*</h5>
